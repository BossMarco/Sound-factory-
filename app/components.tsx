...
export function Header({ locale = "en" }: { locale?: "en" | "es" }) {
  ...
  return <header className="site-header"><Link href={base || "/"} className="brand" aria-label="Sound Factory Productions home"><span className="brand-mark"><Image src="/factory-rgv-mark.jpeg" alt="Factory RGV logo" width={58} height={58} priority unoptimized /></span><span className="brand-copy"><b>SOUND FACTORY</b><i>PRODUCTIONS · RGV</i></span></Link><nav className="desktop-nav" aria-label="Main navigation"><Link href={`${base}/#services`}>{es ? "Servicios" : "Services"}</Link><Link href={`${base}/locations/mcallen`}>{es ? "Ciudades" : "Locations"}</Link><Link className="nav-cta" href={`${base}/#build-package`}>{es ? "Reservar" : "Check your date"}</Link><a className="language-link" href={swap}>{es ? "EN" : "ES"}</a></nav><details className="mobile-menu"><summary aria-label="Open menu">{es ? "Menú" : "Menu"} <span>+</span></summary><div><Link href={`${base}/#services`}>{es ? "Servicios" : "Services"}</Link><Link href={`${base}/locations/mcallen`}>{es ? "Ciudades" : "Locations"}</Link><Link href={`${base}/#build-package`}>{es ? "Consultar fecha" : "Check your date"}</Link><a href={swap}>{es ? "English" : "Español"}</a></div></details></header>;
}
