...
export function Header({ locale = "en" }: { locale?: "en" | "es" }) {
  ...
  return <header className="site-header"><Link href={base || "/"} className="brand" aria-label="Sound Factory Productions home"><span>SOUND</span><b>FACTORY</b><i>PRODUCTIONS</i></Link><nav className="desktop-nav" aria-label="Main navigation"><Link href={`${base}/services/wedding-dj`}>{es ? "Servicios" : "Services"}</Link><Link href={`${base}/locations/mcallen`}>{es ? "Ciudades" : "Locations"}</Link><Link href={`${base}/#build-package`}>{es ? "Reservar" : "Book now"}</Link><a className="language-link" href={swap}>{es ? "EN" : "ES"}</a></nav><details className="mobile-menu"><summary aria-label="Open menu">{es ? "Menú" : "Menu"} <span>+</span></summary><div><Link href={`${base}/services/wedding-dj`}>{es ? "Servicios" : "Services"}</Link><Link href={`${base}/locations/mcallen`}>{es ? "Ciudades" : "Locations"}</Link><Link href={`${base}/#build-package`}>{es ? "Consultar fecha" : "Check your date"}</Link><a href={swap}>{es ? "English" : "Español"}</a></div></details></header>;
}
